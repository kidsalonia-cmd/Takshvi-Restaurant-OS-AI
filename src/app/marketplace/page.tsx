...
type Summary = OutletBreakdown & {
  rows: number;
  outlets?: number;
  breakdown?: OutletBreakdown[];
};
type UploadResult = {
  success: boolean;
  message?: string;
...
type UploadRow = {
  ...
  brandName: string;
  platform: Platform;
  slot: "petpooja_orders" | "petpooja_items" | "zomato_payout" | "swiggy_payout";
  level: "Location" | "Brand";
};
type AnalysisRow = {
  key: string;
  locationName: string;
...
function inferPlatform(name: string): Platform {
  return PLATFORM_BY_BRAND[normalize(name)] || "Zomato";
}
...
export default function MarketplacePage() {
    ...
    () =>
      visibleLocations.flatMap((location) => {
        const petpoojaRows: UploadRow[] = [
          {
            id: `petpooja-orders:${location.id}`,
            locationId: location.id,
            locationName: location.name,
            brandId: "",
            brandName: "Consolidated Online Sales",
            platform: "Petpooja",
            slot: "petpooja_orders",
            level: "Location",
          },
          {
            id: `petpooja-items:${location.id}`,
            locationId: location.id,
            locationName: location.name,
            brandId: "",
            brandName: "Consolidated Item Sales",
            platform: "Petpooja",
            slot: "petpooja_items",
            level: "Location",
          },
        ...
        const brandRows = brands
          .filter((brand) => brand.location_id === location.id)
          .map((brand) => {
  ...
  useEffect(() => {
    ...
    try {
      setResults(JSON.parse(localStorage.getItem(resultStorageKey) || "{}"));
    } catch {
      setResults({});
    }
  ...
  function attach(rowId: string, selected: File[]) {
    ...
    if (!accepted.length) {
      setMessage("Only XLSX, XLS and CSV files are supported.");
      return;
    }
  ...
  async function upload(row: UploadRow) {
    ...
    try {
      ...
      setMessage(
        row.level === "Location"
          ? `${row.locationName}: Petpooja report split into ${count} online outlet analyses.`
          : `${row.brandName} ${row.platform} payout analysed.`,
      );
    } catch (error) {
  ...
  async function clearWeek() {
    ...
    if (!confirm(`Clear marketplace data for ${targets.length} location(s), ${periodStart} to ${periodEnd}?`)) {
      return;
    }
    ...
    try {
      for (const location of targets) {
        ...
        if (!response.ok || !data.success) {
          throw new Error(`${location.name}: ${data.message || "Unable to clear week."}`);
        }
      }
  ...
  const completed = rows.filter(
    (row) => statuses[row.id] === "uploaded" || statuses[row.id] === "not_applicable",
  ).length;
  ...
  const analysedRows = useMemo<AnalysisRow[]>(() => {
    ...
    for (const row of rows) {
      ...
      const items = breakdown.length
        ...
        : [
            {
              outlet: result.restaurantName || row.brandName,
              platform: result.marketplace || row.platform,
              orders: result.summary.orders,
              sales: result.summary.sales,
              payout: result.summary.payout,
              discount: result.summary.discount,
              commission: result.summary.commission,
              aov: result.summary.aov,
              payoutRatio: result.summary.payoutRatio,
            },
          ];

      for (const item of items) {
        const platform = normalize(item.platform).includes("swiggy")
          ? "Swiggy"
          : normalize(item.platform).includes("zomato")
            ? "Zomato"
            : item.platform;
        const key = `${row.locationId}|${normalize(item.outlet)}|${normalize(platform)}`;
        const existing = grouped.get(key) || {
          locationName: row.locationName,
          restaurant: item.outlet,
          platform,
    ...
    return Array.from(grouped.entries())
      .map(([key, item]) => {
        ...
        const pendingPayout = Math.max(
          item.sales - item.payout - item.discount - item.commission,
          0,
        );
        const deductionRate = item.sales ? ((item.sales - item.payout) / item.sales) * 100 : 0;
        const status = !item.sales
          ? "Missing sales"
          : !item.payout
            ? "Payout pending"
            : payoutRatio < 60
  ...
  const totals = analysedRows.reduce(
    (acc, row) => ({
    ...
    { sales: 0, orders: 0, payout: 0, pending: 0 },
  );
      ...
      <div className="mx-auto max-w-[1900px] space-y-5">
        <header className="rounded-3xl bg-slate-950 p-7 text-white">
          <p className="text-sm font-black uppercase tracking-[.2em] text-emerald-400">
            Takshvi Restaurant OS AI
          </p>
          <div className="mt-2 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <h1 className="text-3xl font-black">Weekly Marketplace Upload Matrix</h1>
              <p className="mt-2 text-sm text-slate-300">
                All locations and outlet details load by default. Use the filter only when you need one location.
              </p>
            </div>
            <div className="flex gap-2">
              <Link href="/dashboard/ceo" className="rounded-xl bg-white px-4 py-3 text-sm font-black text-slate-950">
                CEO Dashboard
              </Link>
              <Link href="/integrations/marketplaces" className="rounded-xl bg-emerald-400 px-4 py-3 text-sm font-black text-slate-950">
                Connections
              </Link>
            </div>
          </div>
        ...
        <section className="grid gap-3 rounded-3xl bg-white p-5 shadow-sm md:grid-cols-2 xl:grid-cols-6">
          ...
          <label className="text-xs font-bold text-slate-500">
            ...
            <input
              ...
              className="mt-1 h-9 w-full rounded-lg border px-2 text-sm text-slate-950"
            />
          </label>
          ...
          <div className="rounded-xl bg-emerald-50 px-4 py-3 text-sm font-black text-emerald-800">
            {visibleLocations.length} locations · {progress}% complete
          </div>

          <button
            onClick={() => void clearWeek()}
            disabled={clearing || !locations.length}
            className="h-12 rounded-xl bg-red-50 px-4 font-black text-red-700 disabled:opacity-50"
          >
            {clearing
              ? "Clearing..."
              : locationFilter === "all"
        ...
        {analysedRows.length ? (
          <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            <Metric label="Online Sales" value={money(totals.sales)} />
            <Metric label="Orders" value={String(totals.orders)} />
            <Metric label="Net Payout" value={money(totals.payout)} />
        ...
        <section className="overflow-hidden rounded-3xl bg-white shadow-sm">
          <div className="border-b p-5">
            <h2 className="text-xl font-black">Weekly report uploads</h2>
            <p className="mt-1 text-sm text-slate-500">Every location and outlet is visible by default.</p>
          </div>
                ...
                {rows.map((row) => {
                  ...
                  return (
                    <tr key={row.id} className="border-b align-middle">
                      ...
                      <td className="p-4">
                        <p className="font-black">{row.brandName}</p>
                        <p className="text-xs text-slate-500">
                          {row.level === "Location"
                            ? "One consolidated upload for location"
                            : "Brand-wise payout report"}
                        </p>
                      </td>
                      <td className="p-4">
                        <span
                      ...
                      <td className="p-4">
                        <label
                          ...
                          className={`block min-w-56 cursor-pointer rounded-xl border-2 border-dashed p-3 text-center text-xs font-bold ${
                            dragging === row.id
                        ...
                        >
                          ...
                          <input
                            key={`${row.id}-${inputKeys[row.id] || 0}`}
                            type="file"
                            accept=".xlsx,.xls,.csv"
                            className="sr-only"
                            onChange={(event) => attach(row.id, Array.from(event.target.files || []))}
                          />
                        </label>
                      </td>
                      <td className="max-w-72 p-4">
                        {file ? (
                          ...
                          <span className="text-xs font-bold text-emerald-700">Saved and analysed</span>
                        ) : (
                      ...
                      <td className="p-4">
                        <div className="flex gap-2">
                          ...
                          >
                            {busy === row.id ? "Saving..." : "Save & Analyse"}
                          </button>
                          <button
                            onClick={() =>
                              setStatuses((current) => ({
                                ...current,
                                [row.id]: "not_applicable",
                              }))
                            }
        ...
        <section className="overflow-hidden rounded-3xl bg-white shadow-sm">
          <div className="border-b p-5">
            <h2 className="text-xl font-black">Live outlet performance</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1550px] text-left text-sm">
                ...
                {analysedRows.length ? (
                  analysedRows.map((row) => (
                    <tr key={row.key} className="border-b">
                      <td className="p-4 font-bold">{row.locationName}</td>
                      <td className="p-4 font-bold">{row.restaurant}</td>
                      <td className="p-4 font-black">{row.platform}</td>
                      <td className="p-4">{money(row.sales)}</td>
                      <td className="p-4">{row.orders}</td>
                      <td className="p-4">{money(row.payout)}</td>
                      <td className="p-4">{money(row.aov)}</td>
                      <td className="p-4">{row.payoutRatio.toFixed(1)}%</td>
                      <td className="p-4">{money(row.pendingPayout)}</td>
                  ...
                  <tr>
                    <td colSpan={11} className="p-8 text-center text-slate-500">
                      Upload reports to generate analysis.
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        {message ? (
          <p className="rounded-2xl bg-emerald-50 p-4 text-sm font-bold text-emerald-900">
            {message}
          </p>
        ) : null}
      </div>
