...
const PLATFORM_BY_BRAND: Record<string, Platform> = {
  "wafflelicious": "Zomato",
  "takshvi cafe delight": "Zomato",
  "bowlzaa": "Zomato",
  "sip and snack cafe": "Zomato",
  "sip snack cafe": "Zomato",
  "honeyman": "Zomato",
  "checkmate cheers": "Zomato",
  "honeyman 49": "Swiggy",
  "coffee and chill cafe": "Swiggy",
  "cafe honey delight": "Swiggy",
  "cafe honeyman cpfv": "Zomato",
  "cafe honeyman dhunela": "Swiggy",
};
...
function inferPlatform(brandName: string): Platform {
  return PLATFORM_BY_BRAND[normalize(brandName)] || "Zomato";
}
...
function validFiles(files: FileList | File[]) {
...
export default function MarketplacePage() {
  ...
  const rows = useMemo<UploadRow[]>(() => {
    ...
    const petpoojaRows: UploadRow[] = [
      { id: `petpooja-orders:${locationId}`, locationId, locationName: selectedLocation.name, brandId: "", brandName: "All online brands", platform: "Petpooja", slot: "petpooja_orders", level: "Location" },
      { id: `petpooja-items:${locationId}`, locationId, locationName: selectedLocation.name, brandId: "", brandName: "All online brands", platform: "Petpooja", slot: "petpooja_items", level: "Location" },
    ];
    const brandRows = locationBrands.map((brand) => {
      const platform = inferPlatform(brand.name);
      return {
        id: `${platform.toLowerCase()}:${brand.id}`,
        locationId,
        locationName: selectedLocation.name,
        brandId: brand.id,
        brandName: brand.name,
        platform,
        slot: platform === "Swiggy" ? "swiggy_payout" : "zomato_payout",
        level: "Brand",
      } as UploadRow;
    });
  ...
  useEffect(() => {
    if (!storageKey) return setStatuses({});
    try { setStatuses(JSON.parse(localStorage.getItem(storageKey) || "{}")); } catch { setStatuses({}); }
  }, [storageKey]);

  useEffect(() => {
    if (storageKey) localStorage.setItem(storageKey, JSON.stringify(statuses));
  }, [storageKey, statuses]);
  ...
  async function upload(row: UploadRow) {
    ...
    try {
      ...
      setMessage(`${row.brandName} ${row.platform} report uploaded and analysed.`);
    } catch (error) {
  ...
  async function clearWeek() {
    ...
    if (!confirm(`Clear all marketplace data for ${periodStart} to ${periodEnd}?`)) return;
    setClearing(true);
    try {
      const response = await fetch("/api/marketplace/upload", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ locationId, brandId: "", periodStart, periodEnd, clearAllBrands: true }),
      });
      const data = await response.json() as { success: boolean; message?: string };
      if (!response.ok || !data.success) throw new Error(data.message || "Unable to clear week.");
  ...
  const analysedRows = rows.filter((row) => results[row.id]?.summary);
      ...
      <div className="mx-auto max-w-[1900px] space-y-5">
        <header className="rounded-3xl bg-slate-950 p-7 text-white">
          ...
          <div className="mt-2 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div><h1 className="text-3xl font-black">Weekly Marketplace Upload Matrix</h1><p className="mt-2 text-sm text-slate-300">Upload Petpooja once per location and payout reports brand-wise.</p></div>
        ...
        <section className="grid gap-3 rounded-3xl bg-white p-5 shadow-sm md:grid-cols-2 xl:grid-cols-6">
          <select value={locationId} onChange={(e) => { setLocationId(e.target.value); setFiles({}); setResults({}); }} className="h-12 rounded-xl border px-3"><option value="">Select location</option>{locations.map((location) => <option key={location.id} value={location.id}>{location.name} ({location.code})</option>)}</select>
          ...
          <div className="rounded-xl bg-emerald-50 px-4 py-3 text-sm font-black text-emerald-800">{progress}% weekly progress</div>
          <button onClick={() => void clearWeek()} disabled={clearing || !locationId} className="h-12 rounded-xl bg-red-50 px-4 font-black text-red-700 disabled:opacity-50">{clearing ? "Clearing..." : "Clear Entire Week"}</button>
        </section>

        <section className="overflow-hidden rounded-3xl bg-white shadow-sm">
          <div className="border-b p-5"><h2 className="text-xl font-black">Weekly report uploads</h2><p className="mt-1 text-sm text-slate-500">Spreadsheet-style upload control for every location, café and platform.</p></div>
            ...
            <table className="w-full min-w-[1550px] text-left text-sm">
              ...
              <tbody>
                {rows.length ? rows.map((row) => {
                  ...
                  return <tr key={row.id} className="border-b align-middle last:border-0">
                    <td className="p-4 font-bold">{row.locationName}</td>
                    <td className="p-4"><p className="font-black">{row.brandName}</p><p className="text-xs text-slate-500">{row.level === "Location" ? "Upload once for this location" : "Upload once for this brand"}</p></td>
                    ...
                    <td className="p-4 whitespace-nowrap">{periodStart}</td>
                    <td className="p-4 whitespace-nowrap">{periodEnd}</td>
                    <td className="p-4"><label onDragEnter={(e) => { e.preventDefault(); setDragging(row.id); }} onDragOver={(e) => e.preventDefault()} onDragLeave={() => setDragging(null)} onDrop={(e) => drop(e, row.id)} className={`block min-w-56 cursor-pointer rounded-xl border-2 border-dashed p-3 text-center text-xs font-bold ${dragging === row.id ? "border-emerald-500 bg-emerald-50" : "border-slate-300"}`}>Drop file or click to browse<input key={`${row.id}-${inputKeys[row.id] || 0}`} type="file" accept=".xlsx,.xls,.csv" className="sr-only" onChange={(e) => attach(row.id, Array.from(e.target.files || []))} /></label></td>
                ...
                }) : <tr><td colSpan={9} className="p-10 text-center text-slate-500">Select a location to load the upload matrix.</td></tr>}
              </tbody>
            </table>
          </div>
        </section>
        ...
        <section className="overflow-hidden rounded-3xl bg-white shadow-sm">
          <div className="border-b p-5"><h2 className="text-xl font-black">Detailed live analysis</h2><p className="mt-1 text-sm text-slate-500">Results refresh immediately after each successful upload.</p></div>
          <div className="overflow-x-auto"><table className="w-full min-w-[1300px] text-left text-sm"><thead className="bg-emerald-500 text-slate-950"><tr>{["Location","Restaurant","Platform","Sales","Orders","Payout","AOV","Payout %","Discount","Commission","Week"].map((heading) => <th key={heading} className="p-4 font-black">{heading}</th>)}</tr></thead><tbody>{analysedRows.length ? analysedRows.map((row) => { const result = results[row.id]; return <tr key={row.id} className="border-b last:border-0"><td className="p-4 font-bold">{row.locationName}</td><td className="p-4 font-bold">{result.restaurantName || row.brandName}</td><td className="p-4 font-black uppercase">{result.marketplace || row.platform}</td><td className="p-4">{money(result.summary?.sales)}</td><td className="p-4">{result.summary?.orders || 0}</td><td className="p-4">{money(result.summary?.payout)}</td><td className="p-4">{money(result.summary?.aov)}</td><td className="p-4">{(result.summary?.payoutRatio || 0).toFixed(1)}%</td><td className="p-4">{money(result.summary?.discount)}</td><td className="p-4">{money(result.summary?.commission)}</td><td className="p-4 whitespace-nowrap">{result.periodStart || periodStart} to {result.periodEnd || periodEnd}</td></tr>; }) : <tr><td colSpan={11} className="p-8 text-center text-slate-500">Upload and analyse reports to populate this table.</td></tr>}</tbody></table></div>
        </section>
