...
function currentWeek() { const now = new Date(); const monday = new Date(now); monday.setDate(now.getDate() - (now.getDay() === 0 ? 6 : now.getDay() - 1)); const sunday = new Date(monday); sunday.setDate(monday.getDate() + 6); const iso = (date: Date) => date.toLocaleDateString("en-CA", { timeZone: "Asia/Kolkata" }); return { start: iso(monday), end: iso(sunday) }; }
...
function resultRows(row: UploadRow, result: UploadResult, periodStart: string, periodEnd: string) {
  const breakdown = result.breakdown || result.summary?.breakdown || [];
  if (breakdown.length) return breakdown.map((item, index) => ({ key: `${row.id}:${index}:${item.outlet}:${item.platform}`, locationName: row.locationName, restaurant: item.outlet, platform: item.platform, periodStart: result.periodStart || periodStart, periodEnd: result.periodEnd || periodEnd, ...item }));
  if (!result.summary) return [];
  return [{ key: row.id, locationName: row.locationName, restaurant: result.restaurantName || row.brandName, platform: result.marketplace || row.platform, periodStart: result.periodStart || periodStart, periodEnd: result.periodEnd || periodEnd, outlet: result.restaurantName || row.brandName, orders: result.summary.orders, sales: result.summary.sales, payout: result.summary.payout, discount: result.summary.discount, commission: result.summary.commission, aov: result.summary.aov, payoutRatio: result.summary.payoutRatio }];
}
...
export default function MarketplacePage() {
  ...
  const rows = useMemo<UploadRow[]>(() => {
    ...
    const brandRows = locationBrands.map((brand) => { const platform = inferPlatform(brand.name); return { id: `${platform.toLowerCase()}:${brand.id}`, locationId, locationName: selectedLocation.name, brandId: brand.id, brandName: brand.name, platform, slot: platform === "Swiggy" ? "swiggy_payout" : "zomato_payout", level: "Brand" } as UploadRow; });
  ...
  const resultStorageKey = storageKey ? `${storageKey}:results` : "";
  useEffect(() => {
    if (!storageKey) { setStatuses({}); setResults({}); return; }
  ...
  useEffect(() => { if (resultStorageKey) localStorage.setItem(resultStorageKey, JSON.stringify(results)); }, [resultStorageKey, results]);
  ...
  function attach(rowId: string, selected: File[]) { const accepted = validFiles(selected); if (!accepted.length) return setMessage("Only XLSX, XLS and CSV files are supported."); setFiles((current) => ({ ...current, [rowId]: accepted[0] })); setStatuses((current) => ({ ...current, [rowId]: "pending" })); setMessage(""); }
  ...
  async function upload(row: UploadRow) {
    ...
    try {
      ...
      const outletCount = data.breakdown?.length || data.summary?.outlets || 1; setMessage(row.level === "Location" ? `${row.platform} report split into ${outletCount} online outlet analyses.` : `${row.brandName} ${row.platform} payout analysed.`);
    } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to upload report."); } finally { setBusy(null); }
  }
  function clearRow(row: UploadRow) { setFiles((current) => ({ ...current, [row.id]: undefined })); setResults((current) => { const next = { ...current }; delete next[row.id]; return next; }); setStatuses((current) => ({ ...current, [row.id]: "pending" })); setInputKeys((current) => ({ ...current, [row.id]: Date.now() })); }
  async function clearWeek() {
    ...
    try {
      const response = await fetch("/api/marketplace/upload", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ locationId, brandId: "", periodStart, periodEnd, clearAllBrands: true }) }); const data = await response.json() as { success: boolean; message?: string }; if (!response.ok || !data.success) throw new Error(data.message || "Unable to clear week.");
      if (storageKey) localStorage.removeItem(storageKey); if (resultStorageKey) localStorage.removeItem(resultStorageKey); setStatuses({}); setFiles({}); setResults({}); setInputKeys({}); setMessage(data.message || "Week data cleared.");
  ...
  const rawRows = useMemo(() => rows.flatMap((row) => results[row.id] ? resultRows(row, results[row.id], periodStart, periodEnd) : []), [rows, results, periodStart, periodEnd]);
  const analysedRows = useMemo<AnalysisRow[]>(() => {
    const grouped = new Map<string, { locationName: string; restaurant: string; platform: string; periodStart: string; periodEnd: string; sales: number; orders: number; payout: number; discount: number; commission: number }>();
    for (const item of rawRows) {
      const platform = normalize(item.platform).includes("swiggy") ? "Swiggy" : normalize(item.platform).includes("zomato") ? "Zomato" : item.platform;
      const key = `${normalize(item.restaurant)}|${normalize(platform)}`;
      const existing = grouped.get(key) || { locationName: item.locationName, restaurant: item.restaurant, platform, periodStart: item.periodStart, periodEnd: item.periodEnd, sales: 0, orders: 0, payout: 0, discount: 0, commission: 0 };
      const isPayout = item.key.startsWith("zomato:") || item.key.startsWith("swiggy:");
      if (isPayout) { existing.payout = Math.max(existing.payout, item.payout); existing.discount = Math.max(existing.discount, item.discount); existing.commission = Math.max(existing.commission, item.commission); if (!existing.sales) existing.sales = item.sales; if (!existing.orders) existing.orders = item.orders; }
      else { existing.sales = Math.max(existing.sales, item.sales); existing.orders = Math.max(existing.orders, item.orders); if (!existing.payout) existing.payout = item.payout; existing.discount = Math.max(existing.discount, item.discount); existing.commission = Math.max(existing.commission, item.commission); }
      grouped.set(key, existing);
    }
    return Array.from(grouped.entries()).map(([key, item]) => {
      const aov = item.orders ? item.sales / item.orders : 0; const payoutRatio = item.sales ? (item.payout / item.sales) * 100 : 0; const pendingPayout = Math.max(item.sales - item.payout - item.discount - item.commission, 0); const deductionRate = item.sales ? ((item.sales - item.payout) / item.sales) * 100 : 0;
  ...
  const totals = useMemo(() => analysedRows.reduce((acc, row) => ({ sales: acc.sales + row.sales, orders: acc.orders + row.orders, payout: acc.payout + row.payout, discount: acc.discount + row.discount, commission: acc.commission + row.commission, pending: acc.pending + row.pendingPayout }), { sales: 0, orders: 0, payout: 0, discount: 0, commission: 0, pending: 0 }), [analysedRows]);
  const overallAov = totals.orders ? totals.sales / totals.orders : 0;
  const overallPayout = totals.sales ? (totals.payout / totals.sales) * 100 : 0;
  const topOutlet = analysedRows[0];
  const alerts = analysedRows.filter((row) => row.status !== "Healthy");

  return (
    <main className="min-h-screen bg-slate-100 p-4 text-slate-950 md:p-7"><div className="mx-auto max-w-[1900px] space-y-5">
      <header className="rounded-3xl bg-slate-950 p-7 text-white"><p className="text-sm font-black uppercase tracking-[.2em] text-emerald-400">Takshvi Restaurant OS AI</p><div className="mt-2 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><h1 className="text-3xl font-black">Weekly Marketplace Upload Matrix</h1><p className="mt-2 text-sm text-slate-300">Phase 3 live analysis combines Petpooja sales with Zomato and Swiggy payout reports.</p></div><div className="flex gap-2"><Link href="/dashboard/ceo" className="rounded-xl bg-white px-4 py-3 text-sm font-black text-slate-950">CEO Dashboard</Link><Link href="/integrations/marketplaces" className="rounded-xl bg-emerald-400 px-4 py-3 text-sm font-black text-slate-950">Connections</Link></div></div></header>

      <section className="grid gap-3 rounded-3xl bg-white p-5 shadow-sm md:grid-cols-2 xl:grid-cols-6"><select value={locationId} onChange={(e) => { setLocationId(e.target.value); setFiles({}); setResults({}); }} className="h-12 rounded-xl border px-3"><option value="">Select location</option>{locations.map((location) => <option key={location.id} value={location.id}>{location.name} ({location.code})</option>)}</select><label className="text-xs font-bold text-slate-500">Week start<input type="date" value={periodStart} onChange={(e) => { setPeriodStart(e.target.value); setResults({}); }} className="mt-1 h-9 w-full rounded-lg border px-2 text-sm text-slate-950" /></label><label className="text-xs font-bold text-slate-500">Week end<input type="date" value={periodEnd} onChange={(e) => { setPeriodEnd(e.target.value); setResults({}); }} className="mt-1 h-9 w-full rounded-lg border px-2 text-sm text-slate-950" /></label><div className="rounded-xl bg-slate-50 px-4 py-3 text-sm"><b>{completed}/{rows.length}</b> reports complete<div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-200"><div className="h-full bg-emerald-500" style={{ width: `${progress}%` }} /></div></div></div><div className="rounded-xl bg-emerald-50 px-4 py-3 text-sm font-black text-emerald-800">{progress}% weekly progress</div><button onClick={() => void clearWeek()} disabled={clearing || !locationId} className="h-12 rounded-xl bg-red-50 px-4 font-black text-red-700 disabled:opacity-50">{clearing ? "Clearing..." : "Clear Entire Week"}</button></section>

      {analysedRows.length ? <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-6"><Metric label="Online Sales" value={money(totals.sales)} /><Metric label="Orders" value={String(totals.orders)} /><Metric label="Net Payout" value={money(totals.payout)} /><Metric label="AOV" value={money(overallAov)} /><Metric label="Payout Ratio" value={`${overallPayout.toFixed(1)}%`} /><Metric label="Pending / Gap" value={money(totals.pending)} /></section> : null}

      {analysedRows.length ? <section className="grid gap-4 xl:grid-cols-3"><div className="rounded-3xl bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-wider text-slate-500">Top Online Outlet</p><h3 className="mt-2 text-xl font-black">{topOutlet?.restaurant || "-"}</h3><p className="mt-2 text-3xl font-black text-emerald-700">{money(topOutlet?.sales)}</p><p className="mt-1 text-sm text-slate-500">{topOutlet?.orders || 0} orders · {topOutlet?.platform || ""}</p></div><div className="rounded-3xl bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-wider text-slate-500">Marketplace Deductions</p><p className="mt-2 text-3xl font-black">{money(totals.discount + totals.commission)}</p><p className="mt-1 text-sm text-slate-500">Discount {money(totals.discount)} · Commission {money(totals.commission)}</p></div><div className={`rounded-3xl p-5 shadow-sm ${alerts.length ? "bg-amber-50" : "bg-emerald-50"}`}><p className="text-xs font-black uppercase tracking-wider text-slate-500">Action Alerts</p><p className="mt-2 text-3xl font-black">{alerts.length}</p><p className="mt-1 text-sm text-slate-600">{alerts.length ? "Outlets need payout or deduction review" : "All analysed outlets look healthy"}</p></div></section> : null}
      ...
      <section className="overflow-hidden rounded-3xl bg-white shadow-sm"><div className="border-b p-5"><h2 className="text-xl font-black">Live outlet performance</h2><p className="mt-1 text-sm text-slate-500">Petpooja sales and marketplace payouts are merged outlet-wise. Dine-in and offline orders are excluded.</p></div><div className="overflow-x-auto"><table className="w-full min-w-[1650px] text-left text-sm"><thead className="bg-emerald-500 text-slate-950"><tr>{["Rank","Location","Restaurant / Virtual Brand","Platform","Sales","Orders","Payout","AOV","Payout %","Discount","Commission","Pending / Gap","Deduction %","Status","Week"].map((heading) => <th key={heading} className="p-4 font-black">{heading}</th>)}</tr></thead><tbody>{analysedRows.length ? analysedRows.map((row, index) => <tr key={row.key} className="border-b last:border-0"><td className="p-4 font-black">#{index + 1}</td><td className="p-4 font-bold">{row.locationName}</td><td className="p-4 font-bold">{row.restaurant}</td><td className="p-4 font-black uppercase">{row.platform}</td><td className="p-4">{money(row.sales)}</td><td className="p-4">{row.orders}</td><td className="p-4">{money(row.payout)}</td><td className="p-4">{money(row.aov)}</td><td className="p-4">{row.payoutRatio.toFixed(1)}%</td><td className="p-4">{money(row.discount)}</td><td className="p-4">{money(row.commission)}</td><td className="p-4">{money(row.pendingPayout)}</td><td className="p-4">{row.deductionRate.toFixed(1)}%</td><td className="p-4"><span className={`rounded-full px-3 py-1 text-xs font-black ${row.status === "Healthy" ? "bg-emerald-100 text-emerald-700" : row.status === "High deduction" ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700"}`}>{row.status}</span></td><td className="p-4 whitespace-nowrap">{row.periodStart} to {row.periodEnd}</td></tr>) : <tr><td colSpan={15} className="p-8 text-center text-slate-500">Upload Petpooja and payout reports to generate combined live analysis.</td></tr>}</tbody></table></div></section>

      {alerts.length ? <section className="rounded-3xl bg-white p-5 shadow-sm"><h2 className="text-xl font-black">Priority actions</h2><div className="mt-4 grid gap-3 lg:grid-cols-2">{alerts.map((row) => <div key={`alert-${row.key}`} className="rounded-2xl border border-amber-200 bg-amber-50 p-4"><p className="font-black">{row.restaurant} · {row.platform}</p><p className="mt-1 text-sm text-slate-700">{row.status === "Payout pending" ? `Upload or verify payout report. Sales recorded: ${money(row.sales)}.` : row.status === "High deduction" ? `Payout ratio is only ${row.payoutRatio.toFixed(1)}%. Review commission, discount and deductions.` : "Sales report is incomplete for this outlet."}</p></div>)}</div></section> : null}
      {message ? <p className="rounded-2xl bg-emerald-50 p-4 text-sm font-bold text-emerald-900">{message}</p> : null}
    </div></main>
  );
}

function Metric({ label, value }: { label: string; value: string }) { return <div className="rounded-3xl bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-wider text-slate-500">{label}</p><p className="mt-2 text-2xl font-black">{value}</p></div>; }
