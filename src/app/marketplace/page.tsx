...
type Summary = { rows: number; orders: number; sales: number; payout: number; discount: number; commission: number; aov: number; payoutRatio: number; outlets?: number; breakdown?: OutletBreakdown[] };
...
function currentWeek() { const now = new Date(); const monday = new Date(now); monday.setDate(now.getDate() - (now.getDay() === 0 ? 6 : now.getDay() - 1)); const sunday = new Date(monday); sunday.setDate(monday.getDate() + 6); const iso = (d: Date) => d.toLocaleDateString("en-CA", { timeZone: "Asia/Kolkata" }); return { start: iso(monday), end: iso(sunday) }; }
...
export default function MarketplacePage() {
  ...
  async function upload(row: UploadRow) {
    ...
    try {
      ...
      formData.append("file", file); formData.append("locationId", row.locationId); formData.append("brandId", row.level === "Brand" ? row.brandId : ""); formData.append("uploadSlot", row.slot); formData.append("periodStart", periodStart); formData.append("periodEnd", periodEnd);
  ...
  const analysedRows = useMemo<AnalysisRow[]>(() => {
    ...
    for (const row of rows) {
      const result = results[row.id]; if (!result?.summary) continue;
      ...
      for (const item of items) {
        ...
        else { existing.sales = Math.max(existing.sales, item.sales); existing.orders = Math.max(existing.orders, item.orders); }
